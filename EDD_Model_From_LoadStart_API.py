...
# hist_data_ml_dlv_duration.columns


# In[17]:


# f=hist_data_ml_dlv_duration.copy()
# f['Target']=hist_data_ml_dlv_duration['Target'].apply(lambda x:int(x))
# f[f['Target']==431][['Customer','Target','Source','Destination','Vehicle Type','Gross Weight',
#        'Delivery Date', 'Customer', 'contract_type', 'Transporter',
#        'Base Freight', 'Actual Freight']]


# In[18]:


# v=hist_data_ml_dlv_duration.groupby(['Customer','Source','Destination','Vehicle Type','Transporter']).size().reset_index()
# v[(v.Source=='Jhagadia')&(v.Destination=='Kheda')]


# In[19]:


# v[(v.Source=='Madurai')&(v.Destination=='Tirunelveli')]


# In[20]:


# for i in hist_data_ml_dlv_duration[(hist_data_ml_dlv_duration.Source=='Jhagadia')&(hist_data_ml_dlv_duration.Destination=='Kheda')]['Target']:
#     print(i)
...
# data_df_final['Target']=data_df_final['Target'].apply(lambda x:int(x))


# In[23]:


# data_df_final[data_df_final['Target']==431]


# In[24]:


# v=data_df_final.groupby(['Customer','Source','Destination','Vehicle Type','Transporter']).size().reset_index()
# v[(v.Source=='Jhagadia')&(v.Destination=='Kheda')]
...
# z=(x-y).dt.total_seconds()//60
# dev_df=DataFrame()
# dev_df['dev']=z[z>0]

# Outlier_dev=DataFrame(columns=dev_df.columns)
# a=EDD.remove_outlier(dev_df,'dev')
# Outlier_dev=concat([Outlier_dev,a],0)
# N0_outlier_rows=len(Outlier_dev)
# print ('Outlier Rows :',N0_outlier_rows,'\nOutlier Data Percentage :', (N0_outlier_rows/len(dev_df))*100)
# print ('Final number of rows available after removing Outliers: ',len(dev_df)-N0_outlier_rows)
# df_OL_free_dev=pd.merge(dev_df,Outlier_dev,indicator=True,how='outer').query('_merge=="left_only"').drop('_merge', axis=1)

# plots.dens_box_plot(df_OL_free_dev)


# In[26]:


# Pred_df=pd.read_csv('Comparison_EDD_OLD_NEW.csv')
...
# Load_std=Pred_df.groupby(['Source','Destination'])['error_from_Loadstart'].agg(np.std, ddof=0).reset_index()
# Load_mean=Pred_df.groupby(['Source','Destination'])['error_from_Loadstart'].agg('mean').reset_index()
# Load_std.columns=['Source','Destination','Lane_std_Loadstart_errors']
# Load_mean.columns=['Source','Destination','Lane_mean_Loadstart_errors']
# load_df=pd.merge(Load_mean,Load_std,on=['Source','Destination'],how='inner')
...
# disp_std=Pred_df.groupby(['Source','Destination'])['error_from_Dispatch'].agg(np.std, ddof=0).reset_index()
# disp_mean=Pred_df.groupby(['Source','Destination'])['error_from_Dispatch'].agg('mean').reset_index()
# disp_std.columns=['Source','Destination','Lane_std_dispatch_errors']
# disp_mean.columns=['Source','Destination','Lane_mean_dispatch_errors']
# disp_df=pd.merge(disp_mean,disp_std,on=['Source','Destination'],how='inner')


# In[27]:


# Pred_df1=pd.merge(Pred_df,load_df,on=['Source','Destination'],how='inner')
# Pred_df2=pd.merge(Pred_df1,disp_df,on=['Source','Destination'],how='inner')
# Pred_df2.columns
...
# Pred_df2['Lane_mean_Loadstart_errors']=Pred_df2['Lane_mean_Loadstart_errors'].apply(lambda x:int(x))
# Pred_df2['Lane_std_Loadstart_errors']=Pred_df2['Lane_std_Loadstart_errors'].apply(lambda x:int(x))
# Pred_df2['Lane_mean_dispatch_errors']=Pred_df2['Lane_mean_dispatch_errors'].apply(lambda x:int(x))
# Pred_df2['Lane_std_dispatch_errors']=Pred_df2['Lane_std_dispatch_errors'].apply(lambda x:int(x))
...
# Pred_df2[['Customer', 'Source', 'Destination',
#        'Transporter', 'Vehicle Type', 'Actual Freight', 'Gross Weight','Actual', 'Prediction_LoadStart',
#        'Prediction_From_Dispatch', 'error_from_Loadstart',
#        'error_from_Dispatch', 'Lane_mean_Loadstart_errors',
#        'Lane_std_Loadstart_errors', 'Lane_mean_dispatch_errors',
#        'Lane_std_dispatch_errors']].to_csv('Error_Analysis.csv',index=False)
...
df=data_df_final_.drop(['Indent ID','Loading Start Date'],1)
...
DataFrame(df_num.drop(['Target'],1).columns,columns=['Num_Var']).to_csv(os.path.join(support_file_path,'Num_Var_EDD.csv'),index=False)
DataFrame(df_obj.columns,columns=['Obj_Var']).to_csv(os.path.join(support_file_path,'Obj_Var_EDD.csv'),index=False)
...
pickle.dump(d, open(os.path.join(support_file_path,'label_coder_EDD.sav'), 'wb'))


# In[45]:


df_lab=pd.concat([data_df_final_['Loading Start Date'],df_lab],1)
...
df_lab.to_csv(os.path.join(processed_datafile_path,'df_lab_EDD.csv'),index=False)
...
predictor_matrix=df_lab_ml.drop(['Loading Start Date','Indent ID','Target'],1).values
...
pickle.dump(scaler_X, open(os.path.join(support_file_path,'scaler_X_EDD.sav'), 'wb'))
pickle.dump(scaler_Y, open(os.path.join(support_file_path,'scaler_Y_EDD.sav'), 'wb'))



columns_X=df_lab_ml.drop(['Loading Start Date','Indent ID','Target'],1).columns
...
DataFrame(scaled_df.drop(['Indent ID','Target',],1).columns.to_list(),columns=['variables']).to_csv(os.path.join(support_file_path,'variables.csv'),index=False)
...
best_var_df.to_csv(os.path.join(support_file_path,'variables_best_edd.csv'),index=False)
...
model_edd=ml_model.RF_reg(predictor_scaled_matrix,response_scaled_vector,filename ='edd_model.sav')
...
model_LR=pickle.load(open(os.path.join(model_file_path,'edd_model.sav'), 'rb'))
...
# tsdf=Test_df_.copy()
# tsdf['Prediction_LoadStart']=y_hat
# tsdf['Prediction_LoadStart']=tsdf['Prediction_LoadStart'].apply(lambda x:int(x))
# tsdf['Actual']=y_act
# tsdf=tsdf[['Indent ID','Prediction_LoadStart']]
...
# comp_df2=pd.read_csv('EDD_CLVT.csv')
...
# comp_df4=pd.merge(comp_df3,tsdf,on=['Indent ID'],how='inner')
