...
def getedd(input_query):
    ...
    date_string = "21 January, 2018"
    refrence_date=datetime.strptime(date_string, "%d %B, %Y")
    input_df['creation_age']=round(( input_df['Loading Start Date']-refrence_date ).dt.total_seconds()/(3600*24))
    ...
    Input_df_coded=Input_df_coded[pd.read_csv(os.path.join(support_file_path,'variables_best_edd.csv'))['Lab_variables'].values]
...
@app.route('/')
def home():
    return '<h1>Go to the below link for EDD prediction given load start time.<h1>\n/GetEDD'

@app.route('/GetEDD/LoadStart', methods=['POST'])
...
def my_test_endpoint():
    ...
    try:
        datetime.strptime(input_query['data']['loading start date'].split('.')[0],'%Y-%m-%d %H:%M:%S')
    except:
